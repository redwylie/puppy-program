    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {Object.entries(allExps).map(([cat, items]) => {
          ...
          return (
            <div key={cat} style={{ border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', background: 'var(--surface)' }}>
              <button
                onClick={() => toggleCat(cat)}
                style={{
                  width: '100%', display: 'flex', alignItems: 'center', gap: 10,
                  padding: '12px 16px', background: 'var(--surface-2)', border: 'none',
                  borderBottom: isOpen ? '1px solid var(--border)' : 'none', cursor: 'pointer', textAlign: 'left'
                }}
              >
                    ...
                    {items.map((item) => {
                      ...
                      return (
                        <label key={item} style={{
                          ...
                          background: isChecked ? 'var(--check-bg)' : 'transparent',
